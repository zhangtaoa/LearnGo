...
$(".weatherList ul li").click(function () {
    ...
    "withCredentials" in t || (t = new XDomainRequest), t.open("POST", "http://fb.weather.com.cn/getdatask"), $.getScript("http://wgeo.weather.com.cn/ip/", function () {
        var e = $(".selectCity span").text(), a = $(".weatherList ul li.cur").attr("title"),
...
var cityId = location.href.match(/\d{9}/)[0], timeas = $("#zs_7d_update_time").val(), sjc = timeas.substr(5, 6),
    yues = timeas.substr(5, 2), ris = timeas.substr(8, 2), ss = yues + ris;
...
for (var html = "", windDY = ["无持续风向", "东北风", "东风", "东南风", "南风", "西南风", "西风", "西北风", "北风", "旋转风"], windJB = ["<3级", "3-4级", "4-5级", "5-6级", "6-7级", "7-8级", "8-9级", "9-10级", "10-11级", "11-12级"], qxbm = {
    ...
    var dayNight = "d";
    dayNight = hour3data[0][i].jf.slice(8, 10) >= 20 || hour3data[0][i].jf.slice(8, 10) < 6 ? "n" : "d", html += 1 == i ?
        '<li class="cur">' +
        '<div class="time">' + hour3data[0][i].jf.slice(8, 10) + '时</div>' +
        '<i class="weather housr_icons_w ' + dayNight + hour3data[0][i].ja + '" title="' + qxbm[hour3data[0][i].ja] + '"></i>' +
        '<div class="charts"></div>' +
        '<div class="wind">' + windDY[hour3data[0][i].jd] + '</div>' +
        '<div class="windL">' + windJB[hour3data[0][i].jc] + "</div></li>" : '<li>' +
        '<div class="time">' + hour3data[0][i].jf.slice(8, 10) + '时</div>' +
        '<i class="weather housr_icons_w ' + dayNight + hour3data[0][i].ja + '" title="' + qxbm[hour3data[0][i].ja] + '"></i>' +
        '<div class="charts"></div>' +
        '<div class="wind">' + windDY[hour3data[0][i].jd] + '</div>' +
        '<div class="windL">' + windJB[hour3data[0][i].jc] + "</div>" +
        "</li>"
}
...
var riseHour = parseInt(sunup[0].slice(0, 2)), riseMinutes = parseInt(sunup[0].slice(3, 5)),
    riseAllM = 60 * riseHour + riseMinutes, setHour = parseInt(sunset[0].slice(0, 2)),
    setMinutes = parseInt(sunset[0].slice(3, 5)), setAllM = 60 * setHour + setMinutes, nowData = new Date,
    hours = nowData.getHours(), minutes = nowData.getMinutes(), nowAllM = 60 * hours + minutes;
riseAllM > nowAllM ? ($("#sunSet").html(sunset[0]), $("#sunRise").html(sunup[0]), $("#sunSetDiv").css({"float": "right"}), $("#sunRiseDiv").css({"float": "left"}), $("#sunRiseName").html("今日日出"), $("#sunSetName").html("今日日落")) : nowAllM >= riseAllM && setAllM > nowAllM ? ($("#sunSet").html(sunset[0]), $("#sunRise").html(sunup[1]), $("#sunSetDiv").css({"float": "left"}), $("#sunRiseDiv").css({"float": "right"}), $("#sunRiseName").html("明日日出"), $("#sunSetName").html("今日日落")) : ($("#sunSet").html(sunset[1]), $("#sunRise").html(sunup[1]), $("#sunSetDiv").css({"float": "right"}), $("#sunRiseDiv").css({"float": "left"}), $("#sunRiseName").html("明日日出"), $("#sunSetName").html("明日日落"));
var nowDate = new Date, nowDateHour = parseInt(nowDate.getHours());
nowDateHour >= 8 && 20 > nowDateHour ? ($("#maxTemp").html(eventDay[1] + "℃"), $("#minTemp").html(eventNight[1] + "℃"), $("#maxTempDiv").css({"float": "left"}), $("#minTempDiv").css({"float": "left"})) : ($("#maxTemp").html(eventDay[2] + "℃"), $("#minTemp").html(eventNight[1] + "℃"), $("#maxTempDiv").css({"float": "right"}), $("#minTempDiv").css({"float": "left"}));
var localCityId = "", localCityId1 = "";
null != cityId && (localCityId = cityId.substr(0, 7), localCityId1 = cityId.substr(0, 5)), $.getScript("http://wgeo.weather.com.cn/ip/", function () {
    var t = id.substr(0, 7);
    "1010101" == t || "1010102" == t || "1010103" == t || "1010104" == t || "1011504" == t ? id.substr(0, 5) == localCityId1 ? $(".topBar .togetherWeather").css({display: "block"}) : $(".topBar .togetherWeather").css({display: "none"}) : t == localCityId ? $(".topBar .togetherWeather").css({display: "block"}) : $(".topBar .togetherWeather").css({display: "none"})
});
var skUrl = "http://d1.weather.com.cn/sk_2d/" + cityId + ".html",
    alarmDataUrl = "http://d1.weather.com.cn/dingzhi/_id_.html";
$.ajax({
    type: "GET", url: skUrl, dataType: "script", success: function () {
        if (dataSK && "暂无实况" != dataSK.temp) {
            ...
            if (t) {
                ...
                $(".sk_alarm").css({"margin-left": "25%"})
            }, 500);
            var s = dataSK.weathercode.slice(1, dataSK.weathercode.length), r = new Date,
...
$.ajax({
    ...
    success: function (t) {
        ...
        $("#radarHref").attr({href: i}), $("#radarBtn").attr({href: i}), $("#radarBox").attr({href: i})
    }
}), $.ajax({
    type: "GET",
    url: "http://d1.weather.com.cn/satellite/JC_YT_DL_WXZXCSYT_4A.html",
    dataType: "jsonp",
    timeout: 2e4,
    jsonpCallback: "readerinfo",
    success: function (t) {
    ...
    var arr = eval("alarmDZ" + cityId).w, len = arr.length;
    if (len) {
        setTimeout(function () {
            $("#aqi").css({"margin-left": "5%"})
        }, 500);
        var gradeObj = {"01": "blue", "02": "yellow", "03": "orange", "04": "red", "05": "white"},
            $item = $('<div class="sk_alarm"></div>');
        $.each(arr, function (t, e) {
            var a = "http://www.weather.com.cn/alarm/newalarmcontent.shtml?file=" + e.w11, i = "";
        ...
        var index = 0;
        _circle(), setInterval(_circle, 4e3), $("#aqi").hasClass("dis") ? $(".sk_alarm").css({"margin-left": "25%"}) : $(".sk_alarm").css({"margin-left": "0"})
    } else setTimeout(function () {
        $("#aqi").css({"margin-left": "25%"})
    }, 500)
});
var chartsLiWidth = $("#weatherALL").width() / 24;
$(".rightBtn").click(function () {
    var t = $("#weatherALL").position().left, e = 11 * -chartsLiWidth;
    t > e && ($(".leftBtn").removeClass("dis"), t -= 3 * chartsLiWidth, e >= t && ($(".rightBtn").addClass("dis"), t = e), $("#weatherALL").is(":animated") || ($("#weatherALL").animate({left: t + "px"}), $("#chartLine").animate({left: t + "px"})))
}), $(".leftBtn").click(function () {
    $(".rightBtn").removeClass("dis");
    var t = $("#weatherALL").position().left, e = 0;
    e > t && (t += 3 * chartsLiWidth, t >= e && ($(".leftBtn").addClass("dis"), t = e), $("#weatherALL").is(":animated") || ($("#weatherALL").animate({left: t + "px"}), $("#chartLine").animate({left: t + "px"})))
